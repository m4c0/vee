...
export inline auto begin_cmd_buf_sim_use(VkCommandBuffer cb, bool inherit = false) {
  VkCommandBufferInheritanceInfo inheritance {
  ...
  if (inherit) info.pInheritanceInfo = &inheritance;
  calls::call(vkBeginCommandBuffer, cb, &info);
}
...
export inline auto cmd_push_compute_constants(VkCommandBuffer cb, VkPipelineLayout pl, const void * ptr, unsigned sz, unsigned offset = 0) {
  calls::call(vkCmdPushConstants, cb, pl, VK_SHADER_STAGE_COMPUTE_BIT, offset, sz, ptr);
}
export inline auto cmd_push_fragment_constants(VkCommandBuffer cb, VkPipelineLayout pl, const void * ptr, unsigned sz, unsigned offset = 0) {
  calls::call(vkCmdPushConstants, cb, pl, VK_SHADER_STAGE_FRAGMENT_BIT, offset, sz, ptr);
}
export inline auto cmd_push_vertex_constants(VkCommandBuffer cb, VkPipelineLayout pl, const void * ptr, unsigned sz, unsigned offset = 0) {
  calls::call(vkCmdPushConstants, cb, pl, VK_SHADER_STAGE_VERTEX_BIT, offset, sz, ptr);
}
export inline auto cmd_push_vert_frag_constants(VkCommandBuffer cb, VkPipelineLayout pl, const void * ptr, unsigned sz, unsigned offset = 0) {
...
export template<typename T> inline auto cmd_update_buffer(VkCommandBuffer cb, VkBuffer buf, const T * data, unsigned ofs = 0) {
  calls::call(vkCmdUpdateBuffer, cb, buf, ofs, sizeof(T), data);
}
export template<typename T> inline auto cmd_update_buffer(VkCommandBuffer cb, VkBuffer buf, const T & data, unsigned ofs = 0) {
  calls::call(vkCmdUpdateBuffer, cb, buf, ofs, sizeof(T), &data);
}
